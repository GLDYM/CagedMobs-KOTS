...
public class MobCageRenderer implements BlockEntityRenderer<MobCageBlockEntity> {
    ...
    public void render(MobCageBlockEntity tile, float partialTicks, PoseStack matrix, MultiBufferSource buffer, int combinedLightIn, int combinedOverlayIn) {
        if(tile.getEnvironmentData() != null && !ClientConfig.disableEnvsRender.get()){
            matrix.pushPose();
            matrix.scale(0.74f,0.015f,0.74f);
            matrix.translate(0.17, 5, 0.17);
            BlockState state = tile.getEnvironmentData().getRenderBlock().defaultBlockState();
            ...
            // blockRenderer.renderSingleBlock(tile.getEnvironmentData().getRenderBlock().defaultBlockState(), matrix, buffer, combinedLightIn, combinedOverlayIn);
            matrix.popPose();
        }
        if(tile.hasEntity() && !ClientConfig.disableEntitiesRender.get()){
            matrix.pushPose();
